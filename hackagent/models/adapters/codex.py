...
def _extract_result_text(stdout: str) -> Optional[str]:
    ...
    for line in text.splitlines():
        ...
        if event_type in {"turn.failed", "error"}:
            ...
            raise AdapterInteractionError(f"codex reported an error: {error}")
...
class CodexAgent(SubprocessCLIAgent):
    ...
    def _complete(self, litellm: Any, messages: List[Dict[str, Any]]) -> Any:
        try:
            ...
            return self._custom_handler.completion(
                model=self.litellm_model,
                messages=messages,
            )
