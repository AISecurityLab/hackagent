...
from hackagent.client import catalog_by_id
...
def _display_generic_attack_info(strategy: str) -> None:
    ...
    meta = catalog_by_id()[strategy]
    tags = (
        ", ".join(meta["tag_labels"]) if meta["tag_labels"] else "none (primary only)"
    )

    info_content = f"""[bold]{meta["label"]} Attack Strategy[/bold]

[cyan]Category:[/cyan]
{meta["category_label"]} — {meta["category_description"]}
[cyan]Tags:[/cyan]
{tags}
