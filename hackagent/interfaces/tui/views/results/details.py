...
from hackagent.interfaces.tui.views.results.formatters import (
    _escape,
    _format_local_datetime,
    _format_result_full_details,
)
from hackagent.interfaces.tui.views.results.formatters.run_report import (
...
class ResultsDetailsMixin:
    ...
    :class:`~hackagent.interfaces.tui.views.results.tab.ResultsTab`."""
    ...
    def _show_result_details(self) -> None:
        ...
        if hasattr(run, "results") and run.results:
            ...
            try:
                ...
                _res_page = self.client().results(run_id=_rid, page=1, page_size=500)
        ...
        try:
            ...
            if _att_id:
                ...
                _att_page = self.client().attacks(page=1, page_size=500)
        ...
        if run_results:
            ...
            _trace_session = self.client()
            _traces_by_result: dict[str, list] = {}
            for _r in run_results:
                try:
                    _traces_by_result[str(_r.id)] = _trace_session.traces(_r.id)
                except Exception:
