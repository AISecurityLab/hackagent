...
from hackagent.interfaces.web._static import (
    bundle_source,
    bundle_version,
    find_bundle,
    static_dir,
...
logger = logging.getLogger("hackagent.interfaces.web")
...
def create_app(client) -> Flask:
    ...
    """
    if client is None:
        raise ValueError("a HackAgent session is required")
    ...
    if client.settings.api_key:
        from hackagent.interfaces.web._proxy import create_proxy

        target = client.settings.base_url
        app.register_blueprint(
            create_proxy(target, client.settings.api_key), url_prefix=API_PREFIX
        )
        app.config["HACKAGENT_MODE"] = "remote"
        app.config["HACKAGENT_TARGET"] = target
    else:
        from hackagent.interfaces.web._local_api import create_local_api

        app.register_blueprint(create_local_api(client), url_prefix=API_PREFIX)
