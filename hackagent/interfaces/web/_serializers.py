...
from hackagent.client import result_bucket
...
def agent(record: Any) -> Dict[str, Any]:
    return {
        "id": str(record.id),
        "name": record.name,
...
def attack(record: Any, agent_name: Optional[str] = None) -> Dict[str, Any]:
    return {
        "id": str(record.id),
        "type": record.type,
...
def trace(record: Any) -> Dict[str, Any]:
    return {
        "id": str(record.id),
