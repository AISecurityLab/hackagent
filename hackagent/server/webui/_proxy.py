...
logger = logging.getLogger("hackagent.server.webui.proxy")
...
def resolve_api_key(explicit: Optional[str] = None) -> Optional[str]:
    """Return the API key to proxy with: the explicit one, else the CLI config's."""
    if explicit:
        return explicit
...
__all__ = ["create_proxy", "resolve_api_key"]
