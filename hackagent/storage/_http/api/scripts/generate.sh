...
#
# Two-step generation of hackagent/storage/_http/api/ from the OpenAPI schema.
#
# Step 1 — datamodel-code-generator → hackagent/storage/_http/api/models.py
#           Produces Pydantic v2 BaseModel classes.
#
# Step 2 — openapi-python-client → hackagent/storage/_http/api/<resource>/
#           Produces typed httpx call functions.
#           Generated models/ and boilerplate are discarded; all model
#           imports are rewritten to point at hackagent/storage/_http/api/models.py.
#
# Usage:
#   hackagent/storage/_http/api/scripts/generate.sh [--schema-url <url>] [--schema-file <path>]
#
# Options:
...
API_DIR="${REPO_ROOT}/hackagent/storage/_http/api"
...
# Do NOT edit manually – run hackagent/storage/_http/api/scripts/generate.sh to regenerate."
...
# Copy each resource sub-package (attack/, run/, …) into hackagent/storage/_http/api/
...
# (api/<resource>/ is 2 levels inside hackagent/storage/_http/api/ so ..models = hackagent.storage._http.api.models)
echo ""
