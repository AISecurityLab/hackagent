...
def web(ctx, host, port, db_path, force_local, no_browser):
    ...
    from hackagent.server.webui import MissingBundleError, create_app

    cli_config: CLIConfig = ctx.obj["config"]
    api_key = None if force_local else cli_config.api_key
    ...
    if api_key:
        ...
        console.print("    Mode : [cyan]local (read-only)[/cyan]")
