...
from typing import Dict, Iterable, Literal, Mapping, Optional, Tuple, get_args
...
#: Canonical attack identifiers, in listing order. Every package names an
#: attack by one of these strings; there are no aliases.
AttackId = Literal[
    "baseline",
    "static_template",
    "flipattack",
...
ATTACK_IDS: Tuple[str, ...] = get_args(AttackId)
...
def get_attack_taxonomy(technique_key: str) -> AttackTaxonomy:
    ...
    """
    try:
        return ATTACK_TAXONOMY[technique_key]
        ...
        raise KeyError(
            ...
            "hackagent.catalog.taxonomy.ATTACK_TAXONOMY."
        ) from exc
...
def try_get_attack_taxonomy(technique_key: str) -> Optional[AttackTaxonomy]:
    ...
    return ATTACK_TAXONOMY.get(technique_key)
