...
SDK and CLI launches still use the `attack_config` dict above. Every technique on this page also constructs as [`BaseAttack(config, ctx)`](./seam.md). The legacy `(config, client, agent_router)` constructor is obsolete for new technique code; the orchestrator still calls it.
:::
