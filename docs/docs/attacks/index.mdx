...
graph LR
    subgraph "Attack Types"
        ...
        J[Indirect Prompt Injection] --> |"RAG Poisoning"| T
    end
...
| [**Indirect Prompt Injection**](./indirect_prompt_injection.md) | RAG document poisoning for indirect instruction following | ⭐⭐⭐ High | Medium |
...
## Indirect Prompt Injection — RAG Document Poisoning

A sophisticated attack that tests whether a RAG-augmented agent can be manipulated through **poisoned documents** in its knowledge base. HackAgent controls the entire RAG pipeline internally.
...
attack_config = {
    ...
    "embedder": {"identifier": "text-embedding-3-small", "endpoint": "https://api.openai.com/v1", "api_key": "OPENAI_API_KEY"},
    "poisoning": {"strategy": "inline_context_override", "poisoned_ratio": 0.5},
}
```

[**Learn more about Indirect Prompt Injection →**](./indirect_prompt_injection.md)
...
**Indirect Prompt Injection** is the only attack testing **indirect** vectors: it evaluates whether poisoned documents in a knowledge base can manipulate the agent's responses to innocent users. Use it when assessing RAG pipeline security.
