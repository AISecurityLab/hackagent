...
Crescendo, PAIR, and AdvPrefix still put many attack-specific keys at the top level (`max_turns`, `n_iterations`, `meta_prefixes`). AutoDAN-Turbo, PAP, CipherChat, MML, and others already nest theirs under `*_params`. The long-term convention is one nested `*_params` field per attack; that API change is **not** in this docs-only update.
:::
