...
Every technique has exactly one **primary category** — Static, Adaptive, or Multi-turn — describing how it hits the target. Tags such as **multimodal** and **RAG/indirect** are secondary labels, not extra categories. See [Attack taxonomy](./taxonomy.mdx) for definitions and the assignment table.
...
Grouped by primary category. Speed/sophistication still vary inside a category (Baseline is a control; AdvPrefix is a long search).
...
|--------|-------------|------|
| [**Baseline**](./baseline.md) | Sends goals to the target with no transformation (control condition) | — |
| [**Static Template**](./static-template.md) | Template-based prompt injection | — |
| [**FlipAttack**](./flipattack.md) | Character-level text obfuscation | — |
| [**CipherChat**](./cipherchat.md) | Cipher-based non-natural-language jailbreak prompts | — |
| [**h4rm3l**](./h4rm3l.md) | Composable prompt-decoration chains | — |
| [**MML**](./mml.md) | Image-encoded jailbreak for Vision-Language Models | multimodal |
| [**FC**](./fc.md) | Auto-generated flowchart images to jailbreak VLMs | multimodal |
| [**tFC**](./tfc.md) | Auto-generated flowchart text to jailbreak LLMs | — |
| [**RAG Attack**](./rag.md) | RAG document poisoning for indirect instruction following | indirect, rag |

### Adaptive

Many independent attempts that refine or search, without a shared growing conversation.

| Attack | Description | Tags |
|--------|-------------|------|
| [**PAIR**](./pair.md) | LLM-driven iterative prompt refinement | — |
| [**TAP**](./tap.md) | Tree search with on-topic pruning | — |
| [**PAP**](./pap.md) | Persuasive adversarial paraphrasing with social-science techniques | — |
| [**BoN**](./bon.md) | Best-of-N random text augmentation search | — |
| [**AdvPrefix**](./advprefix.md) | Multi-step adversarial prefix optimization | — |
| [**AutoDAN-Turbo**](./autodan_turbo.md) | Lifelong strategy discovery and reuse | — |
| [**Tool-output IPI**](./tool_output_ipi.md) | Poisoned tool observations (InjecAgent / OPI) | indirect |

### Multi-turn

One growing conversation with the target.

| Attack | Description | Tags |
|--------|-------------|------|
| [**Crescendo**](./crescendo.md) | Multi-turn conversational escalation with backtracking | — |

:::tip Tags are secondary
**multimodal** marks VLM/image attacks (MML, FC). **indirect** / **rag** mark document poisoning (RAG Attack); **indirect** alone also marks tool-observation injection (Tool-output IPI). They never replace Static / Adaptive / Multi-turn.
:::

:::tip Shared configuration
Keys such as `goals`, `dataset`, `intents`, `objective`, `attacker`, `judges`, batching (`batch_size`, `goal_batch_size`, `goal_batch_workers`, `judge_concurrency`), `output_dir`, and `category_classifier` are documented on **[Shared Attack Config](./shared-args.md)**. Attack pages cover technique-specific options and say exactly whether those live in a `*_params` block or at the top level.

Crescendo, PAIR, and AdvPrefix still put many attack-specific keys at the top level (`max_turns`, `n_iterations`, `meta_prefixes`). AutoDAN-Turbo, PAP, CipherChat, MML, and others already nest theirs under `*_params`. The long-term convention is one nested `*_params` field per attack; that layout change is still ahead.

SDK and CLI launches still use the `attack_config` dict above. Every technique on this page also constructs as [`BaseAttack(config, ctx)`](./seam.md). The legacy `(config, client, agent_router)` constructor is obsolete for new technique code. The runner constructs `(config, ctx)`.
:::
...
list.** Full shared-config reference: [Shared Attack Config](./shared-args.md).
...
Start from the [attack taxonomy](./taxonomy.mdx): **static** for a fast first pass, **adaptive** when you can spend queries on search or refinement, **multi-turn** when you need a growing conversation, plus **multimodal** or **indirect**/**rag** tags when the channel matters.
...
**RAG Attack** and **Tool-output IPI** test **indirect** vectors: RAG poisons knowledge-base documents; tool-output IPI poisons tool observations (InjecAgent / OPI). Use them when assessing RAG pipelines or tool-using agents.
...
read success.** The RAG Attack still fills it from its own loop. Post-hoc
techniques return rows from `run()` without a verdict, except AdvPrefix
selection, which calls `ctx.judge.evaluate`. Inline-judge techniques (BoN,
PAP, Tool-output IPI, TAP) score inside the loop with `ctx.judge.score`.
Custom-loop techniques (Crescendo, PAIR, AutoDAN-Turbo, RAG) score with
`ctx.judge` as well. `HackAgent.hack` then runs the shared evaluator and
writes `success` / `best_score` into `metadata`.
...
With more than one judge configured, `success` follows the majority of the
votes cast (a tie is not a success) unless `judge_aggregation` says otherwise.
A judge that failed or gave an unparseable reply abstains: its column is
`None`, not `0`. A row no judge could score has `success: False`, a
`judge_error`, and evaluation status `ERROR_TEST_FRAMEWORK`. See
[Combining several judges](./shared-args.md#combining-several-judges).
...
- [Shared Attack Config](./shared-args.md) — goals, judges, batching, and where `*_params` belong
...
- [CipherChat](./cipherchat.md) — Cipher-based encoding
- [PAP](./pap.md) — Persuasive paraphrasing
