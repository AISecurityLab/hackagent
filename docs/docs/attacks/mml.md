...
`run()` returns generation rows without a verdict. `MMLConfig` still subclasses `ConfigBase`. The legacy constructor `MMLAttack(config_dict, client, agent_router)` is obsolete for new code. The runner constructs `(config, ctx)`.
