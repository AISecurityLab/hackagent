...
| Attack | Category | CLI Command |
|--------|----------|-------------|
| Baseline | Static | `hackagent eval baseline` |
| Static Template | Static | `hackagent eval static-template` |
| FlipAttack | Static | `hackagent eval flipattack` |
| CipherChat | Static | `hackagent eval cipherchat` |
| h4rm3l | Static | `hackagent eval h4rm3l` |
| MML | Static (tag: multimodal) | `hackagent eval mml` |
| FC | Static (tag: multimodal) | `hackagent eval fc` |
| tFC | Static | `hackagent eval tfc` |
| PAIR | Adaptive | `hackagent eval pair` |
| TAP | Adaptive | `hackagent eval tap` |
| PAP | Adaptive | `hackagent eval pap` |
| BoN | Adaptive | `hackagent eval bon` |
| AdvPrefix | Adaptive | `hackagent eval advprefix` |
| AutoDAN-Turbo | Adaptive | `hackagent eval autodan_turbo` |
| Crescendo | Multi-turn | `hackagent eval crescendo` |
| RAG | Static (tags: indirect, rag) | `hackagent eval rag` |

Additionally, `hackagent eval chain` runs a fallback-ladder sequence of multiple attacks against a shared pool of goals (own `--config-file`/`--goals`/`--timeout`/`--dry-run`/guardrail options; see `hackagent eval chain --help`). The quick scan (`hackagent eval` with no attack name) calls `hack_chain` through the facade. Strategy commands are generated from that catalog, so crescendo and rag are included: `hackagent eval rag` runs the indirect-injection technique (primary category **Static**, tags **indirect** / **rag** — see [RAG Attack](../attacks/rag.md)). Categories are defined in [Attack taxonomy](../attacks/taxonomy.mdx); `hackagent eval list` prints the same assignments.
...
```
  ...
  <TabItem value="static_template" label="StaticTemplate">
...
| `n_samples_per_template`, `judges`/`judge`/`judge_config`, `min_response_length`, `deduplicate_responses` | ❌ | Optional static-template settings (LLM judge is always used) |

### Example `static_template.json`
...
|------------|----------|-------|
| `goals` or `dataset` | ✅ | No usable default (`goals=[]`, `dataset=None`), must be provided |
| `objective`, `attacker`, `n_iterations`, `output_dir` | ❌ | Required at runtime but defaulted on `PairConfig` (alias `DEFAULT_PAIR_CONFIG`) |
| `judge`, `n_streams`, `early_stop_on_success`, `jailbreak_threshold` | ❌ | Optional PAIR settings (`range: binary|decimal`) |
...
```
  ...
  <TabItem value="crescendo" label="Crescendo">
...
| `judge`, `summarizer`, `embedder`, `target_request_overrides` | ❌ | Optional extra roles/config (LLM judge always used in evaluation) |
...
- [Evaluation Tutorial](../getting-started/attack-tutorial.mdx) — Step-by-step guide
