...
| Position | Purpose |
|----------|---------|
| `guardrails["before"]` | Inspects the **prompt** before it reaches the target model. Blocks malicious or unsafe inputs. |
| `guardrails["after"]` | Inspects the **response** after the model generates it. Censors harmful outputs. |

Both guardrails are optional and can be used independently or together.

## Configuration

Guardrails are configured on `.target(guardrails={"before": ..., "after": ...})`. This mirrors real-world deployments where a guardrail sits in front of (or behind) the model, and ensures all attacks run against the same defended target:
...
Once set, the guardrails wrap the target model (`hackagent.models.Guarded`) and apply **transparently to every attack** run against this target — no per-attack configuration needed.
...
Guardrail trigger events (which side fired, the reasoning, and flagged categories) are recorded per-step and visible in the local dashboard (`hackagent web`) or `hackagent results show <result_id>` — the return value of `hack()` itself does not expose separate blocked/successful counters.
