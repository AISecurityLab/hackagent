...
**Note:** Once a custom `endpoint` is set, the target does not fall back to an environment variable for the model API key. Pass it on `.target()` as `target_config={"api_key": ...}` or `metadata={"api_key": ...}`. The HackAgent cloud key is separate: `Settings.resolve(api_key=...)`. Without a custom endpoint, `OPENAI_SDK` falls back to `OPENAI_API_KEY`.
