...
agent = HackAgent(
    ...
    agent_type=AgentTypeEnum.OPENAI_SDK,
    target_config={"api_key": "your-azure-api-key"},
)
```

**Note:** Once a custom `endpoint` (base URL) is set, HackAgent does **not** fall back to any environment variable for the API key — you must pass it explicitly, either as `target_config={"api_key": ...}` or as the top-level `api_key=` argument to `HackAgent(...)`. (Without a custom endpoint, `OPENAI_SDK` falls back to `OPENAI_API_KEY` by default — not an Azure-specific variable.)
...
- [HackAgent Evaluation Tutorial](../getting-started/attack-tutorial.mdx)
