...
Add `--fail-fast` to stop at the first failed attack instead of continuing through the remaining ones (default: `--no-fail-fast`).
...
for attack in JAILBREAK_PROFILE.primary_attacks:
    ...
    print(f"{attack.technique}: {results}")
```

`hack()`'s return shape is strategy-dependent — for a single reliable ASR across attacks, use `hackagent results summary` (CLI) or the local dashboard (`hackagent web`) instead of parsing `results` directly.
...
If the scan shows bypasses, continue with [Evaluation Tutorial](./attack-tutorial) to tune and deepen each attack configuration.
