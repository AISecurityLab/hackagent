...
[`hackagent web`](../cli/web.md) serves the HackAgent dashboard locally. It ships
as a separate package so an install that never opens it does not carry the
frontend assets:
...
You can add it at any time without reinstalling anything else. Release binaries
already include it.
