...
- `GOOGLE_ADK`, `CLAUDE_CODE`, `CODEX`, `HERMES`, and `WEB` use dedicated adapter classes in `hackagent/models/adapters/`. The remaining chat-completions-style types are driven generically through `provider_config.py` + `_ChatRegistration`.
