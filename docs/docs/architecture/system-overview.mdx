...
**`hackagent.models.router.AgentRouter`**
- Resolves an `AgentType` (e.g. `GOOGLE_ADK`, `CLAUDE_CODE`, `CODEX`, `HERMES`, `WEB`, `LITELLM`, `OPENAI_SDK`, `OLLAMA`, `LANGCHAIN`) to a provider adapter and dispatches attack prompts to the target agent.
- `GOOGLE_ADK`, `CLAUDE_CODE`, `CODEX`, `HERMES`, and `WEB` use dedicated adapter classes in `hackagent/models/adapters/`; the CLI-driven ones (`CLAUDE_CODE`, `CODEX`, `HERMES`) share the `SubprocessCLIAgent` base in `cli_agent.py`. The remaining chat-completions-style types are driven generically through `provider_config.py` + `_ChatRegistration`.
...
Both backends implement the same `Store` protocol (`hackagent/storage/store.py`), so the router, attack orchestrator, tracker, and TUI/dashboard code are fully decoupled from where results are actually persisted.
