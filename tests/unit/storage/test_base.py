...
class TestStorageBackendProtocol(unittest.TestCase):
    ...
    def test_local_backend_is_runtime_checkable(self):
        """LocalBackend should pass isinstance check against Store."""
        import tempfile
        import os
...
def test_legacy_attack_types_are_read_as_canonical_ids():
    from datetime import datetime, timezone
    from uuid import uuid4

    from hackagent.storage.records import AttackRecord, canonical_attack_type

    def record(attack_type):
        return AttackRecord(
            id=uuid4(),
            type=attack_type,
            agent_id=uuid4(),
    ...
    assert record("AdvPrefix").type == "advprefix"
    assert record("StaticTemplate").type == "static_template"
    assert record("AutoDANTurbo").type == "autodan_turbo"
    assert record("tFC").type == "tfc"
    assert record("pap").type == "pap"
    assert canonical_attack_type("something-new") == "something-new"
