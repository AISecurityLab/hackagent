...
class TestRemoteBackendAsyncWrites(unittest.TestCase):
    def test_all_deferred_writes_land_after_flush_and_close(self):
        ...
        with (
            patch("hackagent.storage.remote.result_trace_create", trace_mod),
            patch("hackagent.storage.remote.result_partial_update", update_mod),
        ):
    ...
    def test_read_flushes_pending_writes(self):
        ...
        with (
            patch("hackagent.storage.remote.result_trace_create", trace_mod),
            patch("hackagent.storage.remote.result_retrieve", retrieve_mod),
        ):
            backend = RemoteBackend(_client())
            rid = uuid4()
            for seq in range(20):
