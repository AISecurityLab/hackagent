...
from hackagent.storage._http.api.apilogs import apilogs_list, apilogs_retrieve
from hackagent.storage._http.client import AuthenticatedClient
from hackagent.core.errors import UnexpectedStatus
from hackagent.storage._http.api.models import APITokenLog
from hackagent.storage._http.api.models import PaginatedAPITokenLogList
...
def test_sync_success(authenticated_client: AuthenticatedClient):
    ...
    with patch(
        "hackagent.storage._http.api.apilogs.apilogs_list.sync_detailed",
...
async def test_asyncio_success(authenticated_client: AuthenticatedClient):
    ...
    with patch(
        "hackagent.storage._http.api.apilogs.apilogs_list.asyncio_detailed",
...
def test_retrieve_sync_success(authenticated_client: AuthenticatedClient):
    ...
    with patch(
        "hackagent.storage._http.api.apilogs.apilogs_retrieve.sync_detailed",
        return_value=mock_detailed_response,
    ) as mock_sync_detailed:
...
async def test_retrieve_asyncio_success(authenticated_client: AuthenticatedClient):
    ...
    with patch(
        "hackagent.storage._http.api.apilogs.apilogs_retrieve.asyncio_detailed",
        new=mock_asyncio_detailed,
    ) as _:
